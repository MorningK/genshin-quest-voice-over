...
이 프로젝트는 FastAPI 기반 웹 서비스(`server.py`)도 제공합니다. SSE(Server-Sent Events)로 프런트엔드가 업로드한 이미지와 선택적 파라미터를 받아 이미지에 OCR을 수행하고, 동일한 SSE 스트림에서 인식 텍스트와 스트리밍 TTS 음성을 반환합니다. 처리 흐름은 데스크톱의 `pipeline.py`와 동일하게 맞췄습니다.
